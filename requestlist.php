                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
                            ...
                            <th class="px-4 py-3">People Number in the Household</th>
                            <th class="px-4 py-3">Children Number</th>
                            <th class="px-4 py-3">Educational Status Of Children</th>
                            <th class="px-4 py-3">Employment Status</th>
                            <th class="px-4 py-3">Expenditures</th>
                            <th class="px-4 py-3">Support Type</th>

                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        <?php // Assuming $result is the result of your query;
                        $query = "SELECT * FROM indigents";
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $indigent_name = $row['indigent_name'];
                            $indigent_lastname = $row['indigent_lastname'];
                            $indigent_phone = $row['indigent_phone'];
                            $indigent_address = $row['indigent_address'];
                            $indigent_income = $row['indigent_income'];
                            $indigent_household = $row['indigent_people_number'];
                            $indigent_children = $row['indigent_children_number'];
                            $education_status_string = $row['indigent_child_educational_status'];
                            $indigent_employment = $row['indigent_employment'];
                            $indigent_expenditures = $row['indigent_expenditures'];
                            $indigent_support = $row['indigent_needed_support'];


                            echo "
                            <tr class='text-gray-700'>
                                <td class='px-4 py-3 text-ms font-semibold border'>$indigent_id</td>
                                <td class='px-4 py-3 border'>
                                    <div class='flex items-center text-sm'>
                                        <div class='relative w-8 h-8 mr-3 rounded-full md:block'>
                                            <img class='object-cover w-full h-full rounded-full' src='img/indigent.png' alt='' loading='lazy' />                                            <div class='absolute inset-0 rounded-full shadow-inner' aria-hidden='true'></div>
                                        </div>
                                        <div>
                                            <p class='font-semibold text-black'>$indigent_name</p>
                                            <p class='text-l fond-bold text-gray-600'>$indigent_lastname</p>
                                        </div>
                                    </div>
                                </td>
                                <td class='px-4 py-3 text-ms font-semibold border'>$indigent_phone</td>
                                <td class='px-4 py-3 text-sm border '>$indigent_address</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_income</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_household</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_children</td>
                                <td class='px-4 py-3 text-sm border'>$education_status_string</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_employment</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_expenditures</td>
                                <td class='px-4 py-3 text-sm border'>$indigent_support</td>
                            </tr>
                            ";
                        }
                        // Free the result set
